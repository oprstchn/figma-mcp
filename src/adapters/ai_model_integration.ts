...
import { McpServer } from "../model/model_context_protocol.ts";
import {
	FigmaModelContextResourceProvider,
	FigmaToModelContextAdapter,
} from "./figma_to_model_context_adapter.ts";
...
export class FigmaMcpServer {
	...
	constructor(
		...
		// Figmaクライアントを初期化
		this.figmaClient = new Client(authConfig);
	...
	private registerResources(): void {
		...
		this.server.resource(
			...
			async (uri, params) => {
				...
				const resource = this.adapter.convertNodeToResource(
					node,
					fileKey,
					nodeId,
				);
		...
		this.server.resource(
			...
			async (uri, params) => {
				...
				const resource = this.adapter.convertComponentToResource(
					response.component,
				);
				return { contents: [resource] };
			},
		...
		this.server.resource(
			...
			async (uri, params) => {
				...
				const response = await this.commentsClient.getComment({
					file_key: fileKey,
					comment_id: commentId,
				});
				const resource = this.adapter.convertCommentToResource(
					response.comment,
					fileKey,
				);
				return { contents: [resource] };
	...
	private registerTools(): void {
					...
					content: [
						{
							type: "text",
							text:
								`検索クエリ "${query}" に一致するFigmaファイルの検索結果です。\n\n` +
		...
		this.server.tool(
			...
			async (params) => {
				...
				try {
					const response =
						await this.componentsClient.getTeamComponents(teamId);
					const filteredComponents = response.components.filter((comp) =>
		...
		this.server.tool(
			...
			async (params) => {
				...
				try {
					...
					const content = this.resourceProvider.convertNodeToContent(
						node,
						fileKey,
						nodeId,
					);
